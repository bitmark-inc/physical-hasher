...
CyU3PDmaBufMgr_t glBufferManager = { {0}
, 0, 0, 0, 0, 0 };
...
void *CyU3PDmaBufferAlloc(uint16_t size) {
	...
	// Find the number of 32 byte chunks required. The minimum size that can be handled is 64 bytes
...
int CyU3PDmaBufferFree(void *buffer) {
	...
	if ((start > glBufferManager.startAddr) && (start < (glBufferManager.startAddr + glBufferManager.regionSize))) {
		...
		while ((wordnum < glBufferManager.statusSize)
		       && ((glBufferManager.usedStatus[wordnum] & (1 << bitnum)) != 0)) {
			count++;
			bitnum++;
			if (bitnum == 32) {
				bitnum = 0;
