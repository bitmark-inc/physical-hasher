...
//    CyU3PBusyWait (200);
	...
	{
		if(g_IsAutoFocus)
			CAMERA_SetAutofocus(g_IsAutoFocus);
	}
        ...
        else if((wIndex & 0x00FF) == ES_UVC_CONTROL_INTERFACE) /* Video Control Interface */
        {
            ...
            if((wIndex == 0x200) && (wValue == 0x200))/*Brightness*/
            {
            	...
            	case ES_UVC_USB_GET_DEF_REQ:
            		...
            		gl16GetControl = CAMERA_GetBrightness(RequestOption);
            	...
            	case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetBrightness((int8_t)gl16SetControl);
					break;
            	}
            }
            ...
            //	CyU3PDebugPrint (4, "\n\rAuto Exposure");
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl8GetControl = CAMERA_GetAutoExposure(RequestOption);
				...
				//	CyU3PDebugPrint (4, "\n\rAuto Exposure= %d",gl8SetControl);
					CAMERA_SetAutoExposure(gl8SetControl);
					break;
				}
			}
            else if((wIndex == 0x200) && (wValue == 0x300))/*Contrast*/
            {
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl16GetControl = CAMERA_GetContrast(RequestOption);
					status = CyU3PUsbSendEP0Data(2,(uint8_t*)&gl16GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetContrast((int8_t)gl16SetControl);
					break;
				}
            }
            else if((wIndex == 0x100) && (wValue == 0x400))/*Manual Exposure*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl32GetControl = CAMERA_GetExposure(RequestOption);
				...
				//	CyU3PDebugPrint (4, "\n\rManual Exposure = %d", gl32SetControl);
					CAMERA_SetExposure(gl32SetControl);
					break;
				}
			}
            else if((wIndex == 0x200) && (wValue == 0x600))/*Hue*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl32GetControl = (int32_t) CAMERA_GetHue(RequestOption);
					status = CyU3PUsbSendEP0Data(4,(uint8_t*)&gl32GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetHue((int8_t)gl32SetControl);
					break;
				}
			}
            else if((wIndex == 0x100) && (wValue == 0x600))/*Manual Focus*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					RequestOption = (bRequest & 0x0F);
					gl16GetControl = CAMERA_GetManualfocus(RequestOption);
					status = CyU3PUsbSendEP0Data(2,(uint8_t*)&gl16GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				//	CyU3PDebugPrint (4, "\n\rManual Focus= %d",gl16SetControl);
					CAMERA_SetManualfocus((uint16_t)gl16SetControl);
					g_IsAutoFocus = 0;
					break;
				}
			}
            else if((wIndex == 0x200) && (wValue == 0x700))/*Saturation*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl16GetControl = CAMERA_GetSaturation(RequestOption);
					status = CyU3PUsbSendEP0Data(2,(uint8_t*)&gl16GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetSaturation((uint32_t)gl16SetControl);
					break;
				}
			}
            else if((wIndex == 0x200) && (wValue == 0x800))/*Sharpness*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl16GetControl = CAMERA_GetSharpness(RequestOption);
					status = CyU3PUsbSendEP0Data(2,(uint8_t*)&gl16GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetSharpness((uint8_t)gl16SetControl);
					break;
				}
			}
            else if((wIndex == 0x100) && (wValue == 0x800))/*Auto Focus*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl8GetControl = CAMERA_GetAutofocus(RequestOption);
				...
				//	CyU3PDebugPrint (4, "\n\rAuto Focus =%d",gl8SetControl);
					CAMERA_SetAutofocus((uint8_t)gl8SetControl);
					g_IsAutoFocus = 1;
					break;
				}
			}
            else if((wIndex == 0x200) && (wValue == 0xA00))/*White Balance manual*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl16GetControl = CAMERA_GetWhiteBalance(RequestOption);
					status = CyU3PUsbSendEP0Data(2,(uint8_t*)&gl16GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetWhiteBalance((uint8_t)gl16SetControl);
					break;
				}
			}
            else if((wIndex == 0x200) && (wValue == 0xB00))/*White Balance Auto*/
			{
				...
				case ES_UVC_USB_GET_DEF_REQ:
					...
					gl16GetControl = CAMERA_GetAutoWhiteBalance(RequestOption);
					status = CyU3PUsbSendEP0Data(2,(uint8_t*)&gl16GetControl);
					if (status != CY_U3P_SUCCESS)
					{
						CyU3PDebugPrint (4, "\n\rUSBStpCB:VCI SendEP0Data = %d", status);
				...
				case ES_UVC_USB_SET_CUR_REQ:
					...
					CAMERA_SetAutoWhiteBalance((uint8_t)gl16SetControl);
					break;
				}
			}
